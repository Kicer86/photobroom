...
TEST_F(PositionsReseterShould, ResetProperItemsWhenChildIsRemoved)
{
...

    {
        ...
        EXPECT_NE(info1.getPosition(), QPoint(-1, -1));            //siblings before removed one should not be reseted
        EXPECT_NE(info2.getPosition(), QPoint(-1, -1));
        EXPECT_EQ(info4.getPosition(), QPoint(-1, -1));            //siblings after removed one should be reseted
        EXPECT_EQ(info5.getPosition(), QPoint(-1, -1));

        //check sizes - should not be touched
        EXPECT_NE(info1.getSize(), QSize(0, 0));
        EXPECT_NE(info2.getSize(), QSize(0, 0));
        EXPECT_NE(info4.getSize(), QSize(0, 0));
        EXPECT_NE(info5.getSize(), QSize(0, 0));
    }
...
TEST_F(PositionsReseterShould, ResetProperItemsWhenChildChanged)
{
    ...
    {
        const ModelIndexInfo& info = *data.cfind(top->index());
        EXPECT_NE(QRect(-1, -1, 0, 0), info.getRect());            // Parent's size should not be reseted
        EXPECT_EQ(QSize(), info.getOverallSize());                 // But its overall rect should
    }

    {
        ...
        EXPECT_NE(QPoint(-1, -1), info1.getPosition());            //siblings before changed one should not be reseted
        EXPECT_NE(QPoint(-1, -1), info2.getPosition());
        EXPECT_EQ(QPoint(-1, -1), info4.getPosition());            //siblings after changed one should be reseted
        EXPECT_EQ(QPoint(-1, -1), info5.getPosition());

        //check sizes - should not be touched
        EXPECT_NE(QSize(0, 0), info1.getSize());
        EXPECT_NE(QSize(0, 0), info2.getSize());
        EXPECT_NE(QSize(0, 0), info4.getSize());
...
TEST_F(PositionsReseterShould, ResetSiblingsWhenItemRemoved)
{
...

    {
        ...
        EXPECT_EQ(QPoint(-1, -1), info2.getPosition());
        EXPECT_EQ(QPoint(-1, -1), info3.getPosition());
        EXPECT_EQ(QPoint(-1, -1), info4.getPosition());
        EXPECT_EQ(QPoint(-1, -1), info5.getPosition());
        
        //check sizes - should not be touched
        EXPECT_NE(QSize(0, 0), info2.getSize());
        EXPECT_NE(QSize(0, 0), info3.getSize());
        EXPECT_NE(QSize(0, 0), info4.getSize());
        EXPECT_NE(QSize(0, 0), info5.getSize());
    }
