...
namespace {
template <std::forward_iterator T> T findLastConsecutive(T first, T last) {
...
FlatModel::FlatModel(QObject *p) : APhotoDataModel(p), m_db(nullptr) {}

void FlatModel::setDatabase(Database::IDatabase *db) {
  if (m_db != nullptr) {
    auto &backend = m_db->backend();
    disconnect(&backend, &Database::IBackend::photosAdded, this,
               &FlatModel::updatePhotos);

    disconnect(&backend, &Database::IBackend::photosRemoved, this,
               &FlatModel::removePhotos);

    disconnect(&backend, &Database::IBackend::photosModified, this,
               &FlatModel::invalidatePhotos);
  }
  ...
  if (m_db != nullptr) {
    auto &backend = m_db->backend();
    connect(&backend, &Database::IBackend::photosAdded, this,
            &FlatModel::updatePhotos);

    connect(&backend, &Database::IBackend::photosRemoved, this,
            &FlatModel::removePhotos);

    connect(&backend, &Database::IBackend::photosModified, this,
            &FlatModel::invalidatePhotos);

    m_translator = std::make_unique<PhotoDeltaFetcher>(*m_db);
    connect(m_translator.get(), &PhotoDeltaFetcher::photoDataDeltaFetched, this,
            &FlatModel::gotPhotoData);
  }
...
const std::vector<Photo::Id> &FlatModel::photos() const { return m_photos; }

const Database::Filter &FlatModel::filter() const { return m_filters; }

Database::IDatabase *FlatModel::database() const { return m_db; }

const APhotoDataModel::ExplicitDelta &
FlatModel::getPhotoData(const QModelIndex &index) const {
...
QVariant FlatModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == PhotoIdRole) {
    ...
    d = QVariant::fromValue<Photo::DataDelta>(
        static_cast<Photo::DataDelta>(data));
  }
...
int FlatModel::columnCount(const QModelIndex &parent) const {
  return parent.isValid()
             ? 0
             : 1; // only top parent has 1 column, all its items have 0
}

QModelIndex FlatModel::parent(const QModelIndex &) const { return {}; }
...
void FlatModel::removePhotos(const std::vector<Photo::Id> &idsToBeRemoved) {
  ...
  rowsOfIds(idsToBeRemoved.begin(), idsToBeRemoved.end(),
            std::back_inserter(rowsToBeRemoved));

  std::ranges::sort(rowsToBeRemoved);

  std::vector<std::pair<int, int>> rangesToBeRemoved;
  findConsecutiveRanges(rowsToBeRemoved.begin(), rowsToBeRemoved.end(),
                        std::back_inserter(rangesToBeRemoved));
...
void FlatModel::invalidatePhotos(const std::set<Photo::Id> &ids) {
  ...
  findConsecutiveRanges(rowsToBeInvalidated.begin(), rowsToBeInvalidated.end(),
                        std::back_inserter(rangesToBeInvalidated));
  ...
  if (rowsToBeInvalidated.size() != ids.size()) {
    ...
    // TODO: currently all photos are being refetched, try to be smarter and
    // only fetch invalidated photos.
    updatePhotos();
  }
}
...
const APhotoDataModel::ExplicitDelta &
FlatModel::photoData(const Photo::Id &id) const {
  ...
  if (it == m_properties.end()) {
    ...
    // insert empty properties so we won't call fetchPhotoData() for this 'id'
    // again
...
void FlatModel::fetchPhotoData(const Photo::Id &id) const {
  if (m_translator)
    m_translator->fetchIds({id}, {Photo::Field::Path, Photo::Field::Flags,
...
void FlatModel::fetchMatchingPhotos(Database::IBackend &backend) {
  ...
  const auto photos =
      backend.photoOperator().onPhotos(view_filters, sort_action);
...
void FlatModel::fetchedPhotos(const std::vector<Photo::Id> &photos) {
  ...
  auto insertPhotosAndUpdate = [this](auto &output, auto &first,
                                      const auto &last) {
  ...
  if (m_photos.empty())
    ...
    while (new_photos_it != last_new_it() || old_photos_it != last_old_it()) {
      if (new_photos_it != last_new_it() &&
          old_photos_it == last_old_it()) // no more old, but still new ones?
      {
        insertPhotosAndUpdate(old_photos_it, new_photos_it, last_new_it());
        continue;
      } else if (new_photos_it == last_new_it() &&
                 old_photos_it !=
                     last_old_it()) // no more new, but still old ones?
      {
        erasePhotos(old_photos_it, last_old_it());
        continue;
      }
      ...
      if (*new_photos_it ==
          *old_photos_it) // same photo on both sides - continue
      {
        ++new_photos_it;
        ++old_photos_it;
      ...
      const auto position_in_old_collection =
          std::find(old_photos_it, last_old_it(), *new_photos_it);

      if (position_in_old_collection ==
          m_photos.end()) // id of photo from new set doesn't exist in old one -
                          // insertion
      {
        ...
        while (last_non_matching_new != last_new_it()) {
          auto it =
              std::find(old_photos_it, last_old_it(), *last_non_matching_new);

          if (it ==
              last_old_it()) // last_non_matching_new not found, keep looking
          {
            ++last_non_matching_new;
            continue;
          } else // last_non_matching_new found in old sequence.
          {
            insertPhotosAndUpdate(
                old_photos_it, new_photos_it,
                last_non_matching_new); // insert block of new photos into old
                                        // sequence
            break;
          }
        }

        if (last_non_matching_new ==
            last_new_it()) // all items after new_photos_it where not found in
                           // old sequence.
        {
          old_photos_it = erasePhotos(
              old_photos_it, last_old_it()); // everything in old sequence is
...
void FlatModel::fetchedPhotoProperties(const Photo::Id &id,
                                       const ExplicitDelta &properties) {
  auto it = m_idToRow.find(id);

  // photo may have been removed from model in the meantime (between
  // fetchPhotoData request and gotPhotoData execution)
  if (it != m_idToRow.end()) {
...
QModelIndex FlatModel::indexForRow(int r) const { return createIndex(r, 0); }
