...
Database::Filter PhotosModelControllerComponent::allFilters() const
{
    ...
    if (m_dates.empty() == false)
    {
        ...
        filters_for_model.push_back( Database::FilterPhotosWithTag(TagTypes::Date, from, Database::ValueMode::GreaterOrEqual, true) );
        filters_for_model.push_back( Database::FilterPhotosWithTag(TagTypes::Date, to, Database::ValueMode::LessOrEqual, true) );
    }
    ...
    if (m_ratingFrom > 0)
        filters_for_model.push_back( Database::FilterPhotosWithTag(TagTypes::Rating, m_ratingFrom, Database::ValueMode::GreaterOrEqual) );

    if (m_ratingTo < 10)
        filters_for_model.push_back( Database::FilterPhotosWithTag(TagTypes::Rating, m_ratingTo, Database::ValueMode::LessOrEqual) );
