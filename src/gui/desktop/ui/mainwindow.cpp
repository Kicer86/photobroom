...
MainWindow::MainWindow(ICoreFactoryAccessor* coreFactory, IThumbnailUtils* thbUtils, QWidget *p): QMainWindow(p),
    ...
    m_completerFactory()
{
    ...
    ui->imagesView->set(m_thumbnailUtils);
    ui->newImagesView->set(&m_completerFactory);
    ui->newImagesView->set(m_thumbnailUtils);
...
{
    SeriesDetection{m_currentPrj->getDatabase(), m_coreAccessor, m_thumbnailUtils, m_currentPrj.get()}.exec();
}
